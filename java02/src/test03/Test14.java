...
public class Test14 {
	...
	private static void load(){
		...
		try {
			...
			System.out.println("데이터 로딩이 실패하였습니다.");
			list.clear();
		} finally {
	...
	private static void save(){
		...
		try{  		
			out = new BufferedWriter(new FileWriter("test14.dat"));
			for(Score13 score:list){
				out.write(score.getName()+", ");
				out.write(score.getKor()+", ");
				out.write(score.getEng()+", ");
				out.write(score.getMath()+"\n");
			}
	...
	private static void doList() {
		int index = 0;
		for (Score13 score : list) {
			System.out.printf("%-3d %-8s %d %d %d\n", 
					index, score.getName(), score.getKor(), score.getEng(), score.getMath());
			index++;
		} 
	}
